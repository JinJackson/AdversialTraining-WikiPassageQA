...
with torch.no_grad():
    for batch in tqdm(train_dataLoader, desc="Iteration"):
        ...
        for input_id, token_type_id, attention, label in zip(input_ids.long(), token_type_ids.long(), attention_mask, labels):  #遍历batch的每一句话
            ...
            for i in range(len(input_id)):
                if input_id[i].item() in special_tokens:  #查看该token是否在特殊tokens里
                    ...
                    new_input = {}  # 用字典来装每一条数据 'origin_loss','input_ids', 'token_type_ids', 'attention_mask', 'label', 'position', 'origin_id', 'loss'
                    new_input['origin_loss'] = origin_loss.item()
                    ...
                    loss = outputs[0]
                    if loss.item() > new_input['origin_loss']:
            ...
            flag = 0
            final_dict = {}
            final_dict['origin_loss'] =origin_loss.item()
            final_dict['input_ids'] = input_id
            final_dict['token_type_ids'] = token_type_id
            final_dict['attention_mask'] = attention
            final_dict['label'] = label
            final_dict['position'] = []
            final_dict['words_sort'] = []
            for dic in all_inputs_for_a_data:

                final_dict['position'] += dic['position']
                final_dict['words_sort'].append(dic['origin_id'])
            attcked_data.append(final_dict)
        ...
        #跳出一个batch
#print(len(attcked_data))
#print(len(final_dict['position']), len(final_dict['words_sort']))
...
#import pdb
with open(f'data/test2.json', 'w') as writer:
